...
void setup() {
  ...
  ps = new RainParticleSystem(ps_origin);
}
...
void draw() {
  ...
  // ps_origin.y = mouseY;
  int userBoxRad = 200;
  rect(mouseX-userBoxRad, mouseY-userBoxRad, userBoxRad*2, userBoxRad*2);
  
  ps_origin.x = random(width);
  ps_origin.y = random(height);  

  while(ps_origin.x > mouseX - userBoxRad && 
        ps_origin.x < mouseX + userBoxRad &&
        ps_origin.y > mouseY - userBoxRad &&
        ps_origin.y < mouseY + userBoxRad){
  ...
  if (frameCount % freq == 0){
    ...
    ps.addParticle(max_radius, min_radius, growth, 
